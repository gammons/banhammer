...
public class Hud {
    ...
    public void resize (int width, int height) {
        hudCamera.viewportWidth = (Constants.VIEWPORT_HEIGHT
                / (float)height) * (float)width;
        hudCamera.update();
        hudCamera.viewportHeight = Constants.VIEWPORT_HEIGHT;
        hudCamera.viewportWidth = (Constants.VIEWPORT_HEIGHT
                / (float)height) * (float)width;
        hudCamera.position.set(hudCamera.viewportWidth / 2,
                hudCamera.viewportHeight / 2, 0);
        hudCamera.update();
    }
    ...
    public void render() {
        ...
        batch.begin();
        font.draw(batch, "testing shit", 0,0);
        batch.end();
    }
